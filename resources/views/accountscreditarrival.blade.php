...
@section('page_heading','Credit Note On Arrival')
@section('section')
...
?>
    ...
    <div class="col-md-14">
        <form role="form" method="POST" action="accountscreditarrival" id="accountscreditarrivalform">
	                ...
	                <select class="form-control" name="seller" onchange="this.form.submit()">
						...
						@if (isset($seller) && count($seller) > 0)
									@foreach ($seller->all() as $sellers)
										@if ($slr ==  $sellers->id)
											<option value="{{ $sellers->id }}" selected="selected">{{ $sellers->cb_name}}</option>
										@else
											<option value="{{ $sellers->id }}">{{ $sellers->cb_name}}</option>
										@endif
...
<script>
	...
	var url = '{{ route('accountscreditarrival.getstockviewarrival',['countryID'=>":id",'credit_no'=>":rf"]) }}';
...
console.log(autosubmit)
	$(document).ready(function (){ 
		if(autosubmit){
			$( "#accountscreditarrivalform" ).submit();
		}
	})
</script>
@endpush