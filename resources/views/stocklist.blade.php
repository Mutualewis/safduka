...
?>
    <div class="col-md-12">
	        <form role="form" method="POST" action="stocklist" id="stocklistform">
				...
				<div class="row">
		            <div class="form-group col-md-12">
		           		<button type="submit" name="submitlotprices" class="btn btn-lg btn-success btn-block">Add/Update Purchase Information</button>
		            </div>
										...
										if (isset($sale_lots) && count($sale_lots) > 0) {
											...
											foreach ($sale_lots->all() as $sale_lots) {
												...
												if ($sale_lots->dnt > 0 ) {
													...
													echo "<input class='form-control' type='hidden' name='hedge[]' value='$sale_lots->hedge'></td>";
													echo "<input class='form-control' type='hidden' name='weight[]' value='$sale_lots->weight'></td>";



													echo "<td>".$total_value."</td>";
									                echo "<td width='15%'><select class='form-control' name='coffee_buyer[$id]'>";
									               		echo "<option></option>";
														if (isset($buyer) && count($buyer) > 0){									
																	foreach ($buyer->all() as $buyers) {
																		if ($sale_cb_id == $buyers->id ) {
																			echo "<option value='$buyers->id' selected='selected'>$buyers->cb_name</option>";
																		}
																		else{
																			echo "<option value='$buyers->id'>$buyers->cb_name</option>";
																		}
																	}
