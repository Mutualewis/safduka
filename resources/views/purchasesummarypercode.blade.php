...
@section('page_heading','Purchase Summary Per Month Per Code')
@section('section')
...
<div class="row">
	<div class="col-md-6 col-md-offset-3" >
			...
			<div class="container">
		        <form action="{{ URL::to('purchasesummarypercode') }}" class="form-horizontal" method="post" enctype="multipart/form-data">
			                ...
			                <select class="form-control" name="month" >
								...
								@if (isset($months) && count($months) > 0)
											@foreach ($months->all() as $value)
												@if ($mthID ==  $value->mth_number)
													<option value="{{ $value->mth_number }}" selected="selected">{{ $value->mth_name}}</option>
												@else
													<option value="{{ $value->mth_number }}">{{ $value->mth_name}}</option>
												@endif
