...
@section('page_heading','Direct Purchase Prices')
@section('section')
...
?>
    <div class="col-md-12">
	        <form role="form" method="POST" action="directpurchasedetails" id="directpurchasedetailsform">
			    ...
			    <div class="row">			
					<div class="col-md-14 col-md-offset-0 pre-scrollable" style="max-height: 800px;">
					        	<input type="hidden" name="_token" value="{{ csrf_token() }}">		
								<h3>Direct Purchase List(Those in red are foul/bad)</h3>
									...
									<?php
										...
										if (isset($sale_lots) && count($sale_lots) > 0) {
											...
											foreach ($sale_lots->all() as $sale_lots) {
												...
												if ($sale_lots->price  != '0.00' && $sale_lots->price != null) {
												...
												if ($sale_lots->dnt > 0 ) {
													...
													// echo "<td><input class='form-control' name='cprice[]' size='5' value= 2.6></td>";

									                echo "<td width='5%'><select class='form-control' name='fob[$id]'>";
														if (isset($fob) && count($fob) > 0){									
																	foreach ($fob->all() as $value) {
																		if ($prc_fob_id == $value->id ) {
																			echo "<option value='$value->id' selected='selected'>$value->fob_price</option>";
																		}
																		else{
																			echo "<option value='$value->id'>$value->fob_price</option>";
																		}
																	}
													...
													if ($sale_lots->status == "Confirmed" || $sale_lots->status == 'Noted') {
...
console.log(autosubmit)
	$(document).ready(function (){ 
		if(autosubmit){
			$( "#directpurchasedetailsform" ).submit();
		}
	})
</script>
@endpush