...
@section('page_heading','Clean Coffee')
...
<?php 
	...
	if (isset($outturns) && $outturns !== NULL && count($outturns) > 0) {
		$pboutt_outturn_number = $outturns->outt_number;
		$pboutt_boutt_dmp_number = $outturns->boutt_dmp_number;
		$cgr_grower = $outturns->cgr_id;
		$csn_season = $outturns->csn_id;

		$outnetweight = $outturns->outt_net_weight;
		//echo $csn_season;
...
?>
	        ...
	        <form role="form" method="POST" action="cleancoffee">
	        	<input type="hidden" name="_token" value="{{ csrf_token() }}">
	        	<?php 
	        	if (count($pboutt_outturn_number) > 0 ) {
	        		?>
		        	<div class="row">
	        	...
	        	<div class="row">
	        		...
	        		<div class="form-group col-md-3">
	                    <div class="input-group custom-search-form">
	                        <input type="text" class="form-control" name="outt_number" style="text-transform:uppercase; " placeholder="Search/Enter Outturn..."  value="{{ old('outt_number'). $pboutt_outturn_number }}"></input>
...
<!-- 	        	<div class="row">
	        	...
	        	<div class="row">
		            ...
		            <div class="form-group col-md-6">
		                <label>Outturn Net Kgs.</label>
		                <input class="form-control"  id="net_kilograms"  name="net_kilograms" oninput="myFunction()" value="{{ old('net_kilograms').$outnetweight  }}">
		            </div>
	            </div>	
<!-- 		            <div class="form-group col-md-3">
		                <label>Class</label>
	...
	<div class="col-md-3 col-md-offset-0" style="margin-left: -200px;">
		...
		<tbody>
			...
			<?php
				...
				if (isset($outturngrades) && count($outturngrades) > 0) {
					// print_r($outturngrades);
					foreach ($outturngrades->all() as $boutturns) {
							// print_r($season->acat_id);
						$total_kilograms += $boutturns->outtgr_net_weight;
						$total_bags += $boutturns->outtgr_bags; 
						$total_pockets += $boutturns->outtgr_pkts; 
						$id = $boutturns->id;
						echo "<tr>";

							if (count($CoffeeGrade) > 0){
								foreach ($CoffeeGrade->all() as $cgrade) {
									if ($boutturns->cgrad_id ===  $cgrade->id) {
										echo "<td>".$cgrade->cgrad_name."</td>";
									}
								}
							}
							...
							echo "<td>".$boutturns->outtgr_bags."</td>";
							echo "<td>".$boutturns->outtgr_pkts."</td>";
							echo "<td>".$boutturns->outtgr_remarks."</td>";
							echo "<td>"."<a href='/grades_delete/{$id}'  class='btn btn-success btn-danger' >Delete</a>";
						echo "</tr>";
