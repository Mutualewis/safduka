...
@section('page_heading','Direct Invoice Details')
@section('section')
...
?>
    <div class="col-md-12">
	        <form role="form" method="POST" action="confirminvoicesdirect" id="confirminvoicesdirectform">
										...
										if (isset($sale_lots) && count($sale_lots) > 0) {
											...
											foreach ($sale_lots->all() as $sale_lots) {
												...
												if ($sale_lots->status != "Withdrawn" ) {
													...
													$summed_price = ($sale_lots->sum_price)/$sale_lots->sum_weight;

													$total_bags += $sale_lots->sum_bags;

													$total_pkts += $sale_lots->sum_pockets;

										    		$total_value = ($sale_lots->sum_weight)/50 * ($summed_price);

													$bric_value = $summed_price + $sale_lots->sl_margin;

													$bric_value = ($sale_lots->sum_weight)/50 * $bric_value;

										    		if ($sale_lots->fobprice != null) {
										    			// $total_value = $total_value + ($sale_lots->weight)/50 *$sale_lots->fobprice;
										    			$bric_value = $bric_value + ($sale_lots->sum_weight)/50 *$sale_lots->fobprice;
										    		}
													...
													if ($sale_lots->invoice_date != NULL) {

														$inv_date = date("m/d/Y", strtotime($sale_lots->invoice_date));
													...
													echo "<td>".$sale_lots->sum_grades."</td>";									
													...
													echo "<td>".$summed_price. "</td>";
													echo "<td>".$total_value. "</td>";
													echo "<td>".$sale_lots->sl_margin. "</td>";
													echo "<td>".$sale_lots->fobprice. "</td>";
													echo "<td>$".$bric_value."</td>";

													echo "<td><input class='form-control' name='invNumber$id' size='10' value='$sale_lots->invoice'></td>";	
													echo "<td><input class='form-control' name='date$id' size='5' value='$inv_date'></td>";	
		        		...
		        		?>
							<div class="row">
					            <div class="form-group col-md-14">
					           		<button type="submit" name="confirminvoicesdirect" class="btn btn-lg btn-success btn-block">Confirm Invoice</button>
					            </div>
...
console.log(autosubmit)
	$(document).ready(function (){ 
		if(autosubmit){
			$( "#confirminvoicesdirectform" ).submit();
		}
	})
</script>
@endpush