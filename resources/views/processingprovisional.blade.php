...
@section('page_heading','Provisional Bulking Instructions')
@section('section')
...
<?php
	...
	if(isset($prdetails)){
		$reference = $prdetails->pbk_reference_name;
		$prc_season = $prdetails->csn_id;
		$contractID = $prdetails->sct_id;
		$prpid = $prdetails->prp_id;
		$date = date("m/d/Y", strtotime($prdetails->pbk_date));
	}
...
?>
    <div class="col-md-12">
	        <form id="stocksForm" role="form" method="POST" action="/processingprovisional">
            	...
            	<div id='green' class='collapse in' >
		        	<div class="row" >
		        		...
		        		<div class="form-group col-md-3">
		        			<label>Provisional Instruction Number</label>
		            ...
		            <div class="row">
			            ...
			            <div class="form-group col-md-3">
			            	<label>Provisional Instruction Date</label>
			        ...
			        <div class="row">
		        		...
		        		<div class="form-group col-md-3">
			                ...
			                <select class="form-control" id="purpose" name="purpose">
			                	<option></option> 
								@if (isset($provisionalPurpose) && count($provisionalPurpose) > 0)
											@foreach ($provisionalPurpose->all() as $value)
												@if ($prpid ==  $value->id)
													<option value="{{ $value->id }}" selected="selected">{{ $value->prp_name}}</option>
												@else
													<option value="{{ $value->id }}">{{ $value->prp_name}}</option>
												@endif
		        		...
		        		<div class="form-group col-md-3">
			                ...
			                <input class="form-control" id="comments" name="comments" type="text" value="{{ old('comments').$comments }}" />
						</div>	
	            ...
	            <div class="row">
					...
					<?php
						...
						if (isset($StockView) && count($StockView) > 0) {
							...
							?>
					            <div class="form-group col-md-3">
					           		<button type="submit" name="submitinstruction" class="btn btn-lg btn-success btn-block">Add/Update Provisional Instruction</button>
					            </div>						
							<?php
					?>
		            <div class="form-group col-md-3">
		           		<button type="submit" name="printprovisionalinstructions" class="btn btn-lg btn-success btn-block">Print Provisional Instruction</button>
		            </div>
		            <div class="form-group col-md-3">
		           		<button type="submit" name="printprovisionalitructionsprices" class="btn btn-lg btn-success btn-block">Print Instruction With Prices</button>
		            </div>
		            <div class="form-group col-md-3">
		           		<button type="submit" name="confirminstructions" class="btn btn-lg btn-danger btn-block" onclick="return confirm('Are you sure you want to confirm this instruction?');">Confirm Instruction</button>
		            </div>
	            </div>
	        	<div class="row">
...
<script>
	...
	var url = '{{ route('processingprovisional.getstockpurchasedview',['countryID'=>":id",'ref_no'=>":rf"]) }}';
    ...
    var table = $('#stocks-table').DataTable({
        ...
        columns: [
            { data: 'stock_id', name: 'tobeprocessed', searchable: false },
            ...
            { data: 'stock_id', name: 'tobewithdrawn' , searchable: false},
        ...
        columnDefs: [
			...
			{targets: 0,
				...
				var ended = table.cell(meta.row,18).data();
				var stockid = $('<div/>').text(data).html();
				if (stockid == "" || stockid == null){
					stockid = table.cell(meta.row,16).data();
					if (ended == null) {
						return '<input type="checkbox" class="chkpurchased" name="tobeprocessedpurchased[]" value="' + stockid + '" >';
					} else {
						var viewedfield = '<input type="hidden" name="tobeprocessedpurchased[]" value="' + stockid + '" >';
			...
			{targets: 7, 
				...
				'render': function (data, type, full, meta, row){
					...
					if (stockid == "" || stockid == null){
						stockid = table.cell(meta.row,16).data();
						return '<input size = "5" style="text-align:center;" type="text" name="'+"cweightpurchased"+ stockid +'" id="'+"cweightpurchased"+ stockid +'" value="' + $('<div/>').text(data).html() + '">';

					} else {
						return '<input size = "5" style="text-align:center;" type="text" name="'+"cweight"+ stockid +'"  id="'+"cweight"+ stockid +'" value="' + $('<div/>').text(data).html() + '">';
					}
			}},
			...
			{targets: 17, 
				...
				'render': function (data, type, full, meta, row){
					...
					if (stockid == "" || stockid == null){
						stockid = table.cell(meta.row,16).data();
						if (ended == null) {
							return '<input type="checkbox" name="tobewithdrawnpurchased[]" value="'+ stockid + '" disabled>';	
						} else {
							return '<input type="checkbox" name="tobewithdrawnpurchased[]" value="'+ stockid + '" >';	
						}

					} else {
						if (ended == null) {
							return '<input type="checkbox" name="tobewithdrawn[]" value="'+ stockid + '" disabled>';	
						} else {
							return '<input type="checkbox" name="tobewithdrawn[]" value="'+ stockid + '" >';	
						}
					}
	    ...
	    fnDrawCallback: function( oSettings ) {
		    ...
		    $(document).ready(function() {
				...
				$('.chk').off('click').on('click', function(){
					...
					for (var i = 0; i<stockArray.length; i++) {
						if (stockArray[i].stockid == selectedID) {
			...
			$('.chkpurchased').off('click').on('click', function(){
