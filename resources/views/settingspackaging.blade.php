...
@section('page_heading','Packaging Settings')
...
?>
	...
	<div class="col-md-6 col-md-offset-0">
	        ...
	        <form role="form" method="POST" action="/settingspackaging">
		        ...
		        <div class="row">
		          ...
		          <div class="row">
		            ...
		            <div class="form-group col-md-6">
		            	<label>Weight</label>
		                <input class="form-control" placeholder="weight" name="weight" value="{{ old('weight') }}" required>
		            </div>		            
	...
	<div class="col-md-6 col-md-offset-0" style="margin-left: -200px;">
		<form role="form" method="POST" action="settingspackaging">
	        	<input type="hidden" name="_token" value="{{ csrf_token() }}">		
				<h3>Packages</h3>
				<table class="table table-striped" id="packaging-table">
				<thead>
				  <tr>
					<th>
						No.
						...
						if (isset($packaging) && count($packaging) > 0) {
							foreach ($packaging as $value) {
								...
								echo "<tr>";
									...
									echo "<td>".$value->pkg_description."</td>";
									echo "<td>".$value->pkg_weight."</td>";
...
@push('scripts')
<script>
var path= "{{URL::to('settingspackaging')}}";
$(document).ready(function (){  
	var table = $('#packaging-table').DataTable({
	});//end datatable
});//end doc ready
	...
	function checkDeletable(e, val){
	  ...
	  var url = '{{ route('settingspackaging.package_delete',['id'=>":id"]) }}';
