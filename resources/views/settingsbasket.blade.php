...
@section('page_heading','Basket Settings')
...
?>
	...
	<div class="col-md-6 col-md-offset-0">
	        ...
	        <form role="form" method="POST" action="/settingsbaskets">
		        ...
		        <div class="row">
		            ...
		            <div class="form-group col-md-6">
		            	<label>Code</label>
		                <input class="form-control" placeholder="Basket code" name="bscode" value="{{ old('bscode') }}" required>
		            </div>		            
	...
	<div class="col-md-6 col-md-offset-0" style="margin-left: -200px;">
		<form role="form" method="POST" action="settingsraw">
	        	<input type="hidden" name="_token" value="{{ csrf_token() }}">		
				<h3>Baskets</h3>
				<table class="table table-striped" id="baskets-table">
				<thead>
				  <tr>
					<th>
						No.
				...
				<tbody>
					...
					<?php
						...
						if (isset($baskets) && count($baskets) > 0) {
							foreach ($baskets as $value) {
								...
								echo "<tr>";
									...
									echo "<td>".$value->bs_description."</td>";
...
@push('scripts')
<script>
var path= "{{URL::to('settingsbaskets')}}";
$(document).ready(function (){  
	var table = $('#baskets-table').DataTable({
	});//end datatable
});//end doc ready
	...
	function checkDeletable(e, val){
	 ...
	 var url = '{{ route('settingsbaskets.basket_delete',['id'=>":id"]) }}';
	 url = url.replace(':id', basketID);
