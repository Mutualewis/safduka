...
use Ngea\seller;
use Ngea\sale_lots;
use Ngea\PurchaseContractSummary;
...
class PurchaseContractSummaryController extends Controller {

    public function purchaseContractSummaryForm (Request $request){
        $id = null;
        $Season = Season::all(['id', 'csn_season']);
        $country = country::all(['id', 'ctr_name', 'ctr_initial']);
        $sale = null;

        return View::make('purchasecontractsummary', compact('id', 'Season', 'country', 'sale'));

    }

    public function purchaseContractSummary (Request $request){
        $id = null;
        $Season = Season::all(['id', 'csn_season']);
        $country = country::all(['id', 'ctr_name', 'ctr_initial']);
        ...
        if (null !== Input::get('viewpdf')) {
           ...
           return $this->displayExcelReport($request);
            ...
            return View::make('purchasecontractsummary', compact('id', 'Season', 'country', 'sale', 'cid', 'csn_season', 'saleid'));  
        }
    ...
    public function displayPDFReport ($request){
        ...
        // Report title
        $title = 'Purchase Contract Summary';
        ...
        $query = PurchaseContractSummary::
                where('ctr_id', $cid)
                ->where('csn_id', $csn_season)
                ->where('sl_id', $saleid)
                ->orderBy('bric');
        ...
        $columns = [
            ...
            'Bric Bags' => function($result) {
                return $result->br_bags;
            },
            'Price/Fifty' => function($result) {
                return $result->price_per_fifty;
            },
        ...
        */
        return PdfReport::of($title, $meta, $queryBuilder, $columns)
                        ...
                        ->editColumns(['Kilos', 'Bric Bags', 'Value', 'Sale_Values'], [
                            'class' => 'left'
                        ])
                        ->showTotal([
                            'Kilos' => 'point',
    ...
    public function displayExcelReport ($request){
        ...
        */
        return ExcelReport::of($title, $meta, $queryBuilder, $columns)
                        ...
                        ->groupBy('QualityCode')
                        // ->stream(); // or download('filename here..') to download pdf
                        ->download('purchasecontractsummary_Season_'.$selectedSeason.'_Sale_'.$selectedSale);
