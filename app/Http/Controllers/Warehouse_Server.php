...
class WarehouseController extends Controller {
    ...
    public function arrivalInformation (Request $request){
		...
		if (Input::get('confirmgrn') !== NULL){ 
			...
			if ($sif_lot != NULL) {
				...
				$cdetails = Helper::getSaleLotsArrivalLot($saleid, $sif_lot, $cid, $season_name);
				if ($cdetails != NULL) {
				...
				$cdetails = Helper::getSaleLotsArrivalOutturn($saleid, $outt_number, $cid, $season_name);
				if ($cdetails != NULL) {
			...
			if ($stdetails != NULL) {
				...
				->update(['gr_id' => $grnid,'st_dispatch_net' => $dispatch_kilograms,'st_gross' => $delivery_kilograms, 'st_moisture' =>  $moisture,  'pkg_id' =>  $packaging, 'usr_id' =>  $user, 'sts_id' => '1', 'ctr_id' => $cid
					, 'bs_id' => $pr_bsid, 'ibs_id' => $pr_ibsid, 'prc_price' => $pr_price, 'br_id' => $pr_brid, 'sl_id' => $st_slid, 'cgrad_id' => $st_grid, 'st_name' => $st_name]);
				...
				$stid = Stock::insertGetId(['prc_id' => $prdetails->id,'gr_id' => $grnid,'st_dispatch_net' => $dispatch_kilograms,'st_gross' => $delivery_kilograms, 'st_moisture' =>  $moisture,  'pkg_id' =>  $packaging, 'usr_id' =>  $user, 'sts_id' => '1', 'ctr_id' => $cid, 'bs_id' => $pr_bsid, 'ibs_id' => $pr_ibsid, 'prc_price' => $pr_price, 'br_id' => $pr_brid, 'sl_id' => $st_slid, 'cgrad_id' => $st_grid, 'st_name' => $st_name]);
				$request->session()->flash('alert-success', 'Stock Information Updated!!');
   ...
   public function stuffingForm (Request $request){
        ...
        return View::make('stuffing', compact('id', 'Season', 'country', 'shipmentmonth','client', 'shipmentyear'));
    ...
    public function stuffing (Request $request){
        ...
        if (NULL !== Input::get('createsalescontract')){
            ...
            return View::make('stuffing', compact('id', 'Season', 'country', 'cid', 'contract', 'shipmentmonth','client', 'clid', 'spid', 'disposaldate', 'description', 'packages', 'bskt', 'basket', 'Packaging', 'packaging_method', 'packaging_type', 'weighbridge_ticket','StuffingView', 'shipmentyear', 'syrid', 'SalesContract', 'SalesContractSummary', 'client_reference'));
            ...
            if ($sale_contract_id == null ) {
                ...
                return View::make('stuffing', compact('id', 'Season', 'country', 'cid', 'contract', 'shipmentmonth','client', 'clid', 'spid', 'date', 'disposaldate', 'description', 'packages', 'SalesContract', 'bskt', 'basket', 'Packaging', 'packaging_method', 'packaging_type', 'weighbridge_ticket','StuffingView', 'shipmentyear', 'syrid', 'SalesContractSummary', 'client_reference'));            
            }
            ...
            if ($SalesContract != null) {
            	...
            	$client = Client::where('cl_id', $SalesContract->cl_id)->first(); 

            	$client = $client->cl_name; 

            	$ship_month = strtotime($SalesContract->sct_start_date);

            	$ship_month = date('M', $ship_month);
            }

            $data = array('name'=>"Trading Department", "contract"=>$contract, "client"=>$client, "ship_month"=>$ship_month);    

            Mail::send(['text'=>'mail'], $data, function($message) {
             $message->to('trading.ea@nkg.coffee', 'Stuffing-')->subject
                ('Stuffed Contract');
            ...
            return View::make('stuffing', compact('id', 'Season', 'country', 'shipmentmonth','client', 'bskt', 'basket', 'Packaging', 'packaging_method', 'packaging_type', 'weighbridge_ticket','StuffingView', 'shipmentyear', 'syrid', 'SalesContractSummary', 'client_reference', 'cid'));
        }
