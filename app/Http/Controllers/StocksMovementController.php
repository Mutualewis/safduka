...
class StocksmovementController extends Controller {

    public function stockmovementForm (Request $request){
        $id = null;
        ...
        return View::make('stockmovement', compact('id', 'Season', 'country', 'sale', 'months', 'movementInstructionType'));
    ...
    public function stockmovement (Request $request){
        ...
        $months = Months::all(['id', 'mth_name', 'mth_number']);
        $mthID = Input::get('month');
        $movementInstructionType = MovementInstructionType::all(['id', 'mit_name']);

        $instruction_status = Input::get('instruction_status');

        $date = Input::get('date');
        $date_second = Input::get('date_second');
        $movementTypeID = Input::get('MovementType');
        ...
        if (null !== Input::get('viewpdf')) {
           ...
           return $this->displayExcelReport($request);
            ...
            $sale = Sale::where('ctr_id', '=', Input::get('country'))->where('csn_id',  Input::get('sale_season'))->orderBy('sl_no')->get();
            return View::make('stockmovement', compact('id', 'Season', 'country', 'sale', 'cid', 'csn_season', 'saleid', 'months', 'mthID', 'movementInstructionType', 'date', 'date_second', 'movementTypeID', 'instruction_status'));  
        }
    ...
    public function displayPDFReport ($request){
        ...
        // $selectedMonth = Months::where('id', $mthID)->first();
        // $selectedMonth = $selectedMonth->mth_name;
        ...
        $date_second = Input::get('date_second');
        $date_second            = date_create($date_second);
        $date_second            = date_format($date_second, "d-m-Y");

        $movementTypeID = Input::get('MovementType');
        $movementType = MovementInstructionType::where('id',$movementTypeID)->first();
        $movementType = $movementType->mit_name;
        ...
        if ($instruction_status == 1) {
            $movement_status = 0;
            $movement_name = "Instructed";
        
        } elseif ($instruction_status == 2) {
            $movement_status = 1;
            $movement_name = "Confirmed";
        }

        // // Report title
        $title = $movement_name.' '.$movementType . ' Movement ';
        ...
        $query = StockMovement::where('mit_name', $movementType)
                    ->whereBetween('created_at',array($date, $date_second))
                    ->where('movement_status', $movement_status);
                // ->where('csn.csn_season', $selectedSeason)
                // ->whereRaw('MONTH(sale_sl.sl_date) = ?', [$mthID])
        ...
        if ($instruction_status == 1) {
            ...
            $columns = [
                'Season' => function($result) {
                    return $result->csn_season;
                },
                ...
                'To' => function($result) {
                    return $result->wr_name.'-'.$result->loc_row.$result->loc_column.$result->btc_zone;
                },
                'Reason' => function($result) {
                    return $result->reason;
                },
                'Reference' => function($result) {
                    return $result->insloc_ref;
                },
        ...
        */
        return PdfReport::of($title, $meta, $queryBuilder, $columns)
                        ...
                        // ->download('stockmovement_Season_'.$selectedSeason.'_Sale_'.$selectedSale);
    ...
    public function displayExcelReport ($request){
        ...
        // // Report title
        $title = 'Purchase Summary Per Month Per Code';
        ...
        */
        return ExcelReport::of($title, $meta, $queryBuilder, $columns)
                        ...
                        ->groupBy('QualityCode')
                        // ->stream(); // or download('filename here..') to download pdf
                        ->download('stockmovement_Season_'.$selectedSeason.'_Month_'.$selectedMonth);
