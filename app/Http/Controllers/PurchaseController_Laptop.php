...
class PurchaseController extends Controller
{
        ...
        if (null !== Input::get('submitlot')) {
            ...
            if ($pdetails != null) {
                ...
                purchase::where('id', '=', $pid)
                    ->update(['cfd_id' => $coffeeid, 'cb_id' => $coffee_buyer, 'prc_price' => $price, 'sst_id' => $sale_status, 'bs_id' => $basket]);
                ...
                purchase::insert(
                    ['cfd_id' => $coffeeid, 'cb_id' => $coffee_buyer, 'prc_price' => $price, 'sst_id' => $sale_status, 'bs_id' => $basket]);
                    ...
                    if (!empty($data) && $data->count()) {
                        ...
                        foreach ($data as $key => $value) {
                            ...
                            if ($lot != null) {
                                ...
                                if (!empty($cdetails)) {
                                    $insert[] = ['cfd_id' => $cdetails->id, 'cb_id' => $buyerID, 'prc_price' => $price, 'prc_confirmed_price' => $price, 'bs_id' => $basketID, 'sst_id' => $statusID];
                                } else {
                                    return redirect('stockupload')
    ...
    public function confirmPurchases(Request $request)
    {
        ...
        if (null !== Input::get('submitlot')) {
            ...
            if ($pdetails != null) {
                $pid = $pdetails->id;
                purchase::where('id', '=', $pid)
                    ->update(['cb_id' => $coffee_buyer, 'prc_price' => $price, 'prc_confirmed_price' => $confirmed_price, 'sst_id' => $sale_status, 'bs_id' => $basket, 'prc_warrant_no' => $warrant_no, 'prc_warrant_weight' => $warrant_weight, 'prc_purchase_comments' => $comments]);
                $request->session()->flash('alert-success', 'Purchase Information Updated!!');
                ...
                purchase::insert(
                    ['cfd_id' => $coffeeid, 'cb_id' => $coffee_buyer, 'prc_price' => $price, 'prc_confirmed_price' => $confirmed_price, 'sst_id' => $sale_status, 'bs_id' => $basket, 'prc_warrant_no' => $warrant_no, 'prc_warrant_weight' => $warrant_weight, 'prc_purchase_comments' => $comments]
                );
                $request->session()->flash('alert-success', 'Purchase Information Added!!');
    ...
    public function confirmPurchasesList(Request $request)
    {
        ...
        if (null !== Input::get('submitlot')) {
            ...
            for ($i = 0; $i < count($lotid); $i++) {
                ...
                $pid      = $pdetails->id;
                purchase::where('id', '=', $pid)
                    ->update(['prc_confirmed_price' => $cprice[$i], 'sst_id' => null]);
            }
            ...
            //     $pdetails = purchase::where('cfd_id', $confirmed[$i])->first();
            //     $pid = $pdetails->id;
            //     purchase::where('id', '=', $pid)
            //         ->update(['sst_id' =>  '2']);
            // }
            ...
            //     $pdetails = purchase::where('cfd_id', $withdrawn[$i])->first();
            //     $pid = $pdetails->id;
            //     purchase::where('id', '=', $pid)
            //         ->update(['sst_id' =>  '3']);
            // }
                ...
                if ($request->has('sale_season') & Input::get('sale_season') !== "Sale Season") {
                    ...
                    if ($request->has('sale') && Input::get('sale') != 'Sale No.' && Input::get('sale') != 'No Sale Found') {
                        ...
                        if (!isset($sale->sl_quality_confirmedby)) {
                            ...
                            $saleid     = Input::get('sale');
                            if ($request->has('seller') && Input::get('seller') != null) {
                                $sale_lots = sale_lots::where('slid', $saleid)->where('slrid', Input::get('seller'))->where('cbid', Input::get('coffee_buyer'))->whereNull('bric')->get();
                                // $lot_in_sale = sale_lots::where('slid', $saleid)->where('slrid',Input::get('seller'))->where('cbid',Input::get('coffee_buyer'))->where('lot', $lot)->first();
                                // $cdetails = coffee_details::where('id', $lot_in_sale->id)->first();

                            } else {
                                $sale_lots = sale_lots::where('slid', $saleid)->where('cbid', Input::get('coffee_buyer'))->whereNull('bric')->get();
                                // $lot_in_sale = sale_lots::where('slid', $saleid)->where('cbid',Input::get('coffee_buyer'))->where('lot', $lot)->first();
                                // $cdetails = coffee_details::where('id', $lot_in_sale->id)->first();
    ...
    public function confirmInvoices(Request $request)
    {
        ...
        if (null !== Input::get('confirminvoices')) {
            ...
            // $sale_lots = sale_lots::where('slid', $saleid)->where('slrid',Input::get('seller'))->where('cbid',Input::get('coffee_buyer'))->get();
            $sale_lots = sale_lots::where('slid', $saleid)->where('slrid', Input::get('seller'))->where('cbid', Input::get('coffee_buyer'))->where('status', 'Confirmed')->whereNull('invoice')->get();
            ...
            for ($i = 0; $i < count($invoicelot); $i++) {
                ...
                // print_r($invoicelot[$i]);

                purchase::where('id', '=', $pid)
                    ->update(['inv_id' => $inv_id]);

                $request->session()->flash('alert-success', 'Purchase Invoice Information Updated!!');
                Activity::log('Updated Invoice for coffeeid' . $invoicelot[$i] . ' with invoice id ' . $inv_id);
            }
    ...
    public function confirmPaymnet(Request $request)
    {
        ...
        if (null !== Input::get('confirmpayment')) {
            ...
            //     $saleid = $value->id;
            //     $cdetails = coffee_details::where('id', $saleid)->first();
            //     $coffeeid = $cdetails->id;

            //     $pdetails = purchase::where('cfd_id', $coffeeid)->first();
            //     $pid = $pdetails->id;
            //     $invdetails = invoices::where('prc_id', $pid)->first();

            //     if($invdetails != NULL){
            //         invoices::where('id', '=', $pid)
            //         ->update(['inv_no' => $invoice,  'inv_date' =>  $date, 'inv_confirmedby' =>  $user]);
            //         $request->session()->flash('alert-success', 'Purchase Information Updated!!');

            //         Activity::log('Updated purchase invoice information with invoice no. '.$invoice. ' date '. $date. ' confirmed by '. $user);

            //     } else {
            //         invoices::insert(
            //         ['prc_id' => $pid, 'inv_no' => $invoice,  'inv_date' =>  $date, 'inv_confirmedby' =>  $user]);
            //     }
            ...
            // $invoiced = sale_lots::where('slid', $saleid)->where('slrid',Input::get('seller'))->where('cbid',Input::get('coffee_buyer'))->whereNotNull('invoice')->groupBy('invoice')->get();

            $invoiced  = sale_lots::where('slctrid', Input::get('country'))->where('slrid', Input::get('seller'))->whereNotNull('invoice')->whereNull('py_no')->groupBy('invoice')->get();
            $sale_lots = sale_lots::where('invoice', Input::get('invoiced'))->whereNotNull('invoice')->whereNull('py_no')->get();
                    ...
                    if ($request->has('sale') && Input::get('sale') != 'Sale No.') {
                        ...
                        if ($sale->sl_quality_confirmedby == null) {
                            ...
                            if ($request->has('invoiced')) {
                                ...
                                $saleid     = Input::get('sale');
                                $inv        = Input::get('invoiced');
                                $invoiced   = sale_lots::where('slctrid', Input::get('country'))->where('slrid', Input::get('seller'))->whereNotNull('invoice')->groupBy('invoice')->get();
                                // print_r($invoiced);
                                $sale_lots = sale_lots::where('slid', $saleid)->where('slrid', Input::get('seller'))->where('cbid', Input::get('coffee_buyer'))->where('invoice', Input::get('invoiced'))->whereNotNull('invoice')->get();
                                // print_r($inv);
    ...
    public function bricContracts(Request $request)
    {
        ...
        if (null !== Input::get('submitbric')) {
            ...
            if ($brcdetails != null) {
                $br_id = $brcdetails->id;
                bric::where('id', '=', $br_id)
                    ->update(['br_no' => $bric, 'br_date' => $date, 'br_confirmedby' => $user]);
                Activity::log('Updated bric information with bric no. ' . $bric . ' date ' . $date . ' confirmed by ' . $user);
                $request->session()->flash('alert-success', 'Bric Information Updated!!');

            } else {
                $br_id = bric::insertGetId(
                    ['br_no' => $bric, 'br_date' => $date, 'br_confirmedby' => $user]);
                $request->session()->flash('alert-success', 'Bric Information Added!!');
                Activity::log('Inserted bric information with bric no. ' . $bric . ' date ' . $date . ' confirmed by ' . $user);
            }
            $coffeeid = null;
    ...
    public function warrants(Request $request)
    {
        ...
        if (null !== Input::get('submitlot')) {
            ...
            // $sale_lots = sale_lots::where('slid', $saleid)->get();
            if ($request->has('seller') && Input::get('seller') != null) {
                $sale_lots = sale_lots::where('slid', $saleid)->where('slrid', Input::get('seller'))->where('cbid', Input::get('coffee_buyer'))->whereNotNull('payment')->get();

            } else {
                $sale_lots = sale_lots::where('slid', $saleid)->where('cbid', Input::get('coffee_buyer'))->whereNotNull('payment')->get();
    ...
    public function releaseInstructions(Request $request)
    {
        ...
        if (null !== Input::get('submitlot')) {
            ...
            return View::make('releaseinstructions', compact('id',
                'Season', 'country', 'cid', 'csn_season', 'sale', 'CoffeeGrade', 'Warehouse', 'Mill', 'Certification', 'seller', 'sale_lots', 'saleid', 'greensize', 'greencolor', 'greendefects', 'processing', 'screens', 'cupscore', 'rawscore', 'buyer', 'sale_status', 'basket', 'slr', 'sale_cb_id', 'transporters', 'trp', 'release_no', 'date', 'sale_lots_released', 'ref_no', 'rl_date'));
            ...
            $toatt = seller::where('id', '=', Input::get('seller'))->first();
            $to    = null;
            $att   = null;
            if ($toatt !== null) {
                $to  = $toatt->slr_name;
                ...
                if ($request->has('sale_season') && Input::get('sale_season') !== "Sale Season" && $request->has('seller')) {
                    ...
                    if ($request->has('sale') && Input::get('sale') != 'Sale No.') {
                        ...
                        if ($sale->sl_quality_confirmedby == null) {
                            ...
                            return View::make('releaseinstructions', compact('id',
                                'Season', 'country', 'cid', 'csn_season', 'sale', 'CoffeeGrade', 'Warehouse', 'Mill', 'Certification', 'seller', 'sale_lots', 'saleid', 'greensize', 'greencolor', 'greendefects', 'processing', 'screens', 'cupscore', 'rawscore', 'buyer', 'sale_status', 'basket', 'slr', 'sale_cb_id', 'transporters', 'trp', 'ref_no', 'rl_date'));
                        } else {
                            // $sale = Sale::where('ctr_id', '=', Input::get('country'))->where('csn_id',  Input::get('sale_season'))->get();
                            // $sale = Sale::where('ctr_id', '=', Input::get('country'))->where('csn_id',  Input::get('sale_season'))->where('sltyp_id',  '1')->get();
                            $sale = Sale::where('ctr_id', '=', Input::get('country'))->where('csn_id', Input::get('sale_season'))->where('sltyp_id', '1')->where('sl_catalogue_confirmedby', '!=', null)->where('sl_quality_confirmedby', '!=', null)->where('sl_auction_confirmedby', '!=', null)->get();
                            ...
                            // else {
                            //     $sale_lots = sale_lots::where('slid', $saleid)->where('cbid',Input::get('coffee_buyer'))->whereNotNull('warrant_no')->get();
                            //     $lot_in_sale = sale_lots::where('slid', $saleid)->where('cbid',Input::get('coffee_buyer'))->where('lot', $lot)->first();
                            //     $cdetails = coffee_details::where('id', $lot_in_sale->id)->first();
                            ...
                            return View::make('releaseinstructions', compact('id',
                                'Season', 'country', 'cid', 'csn_season', 'sale', 'CoffeeGrade', 'Warehouse', 'Mill', 'Certification', 'seller', 'sale_lots', 'saleid', 'greensize', 'greencolor', 'greendefects', 'processing', 'screens', 'cupscore', 'rawscore', 'buyer', 'sale_status', 'basket', 'slr', 'sale_cb_id', 'transporters', 'trp', 'release_no', 'sale_lots_released', 'date', 'ref_no', 'rl_date'));
                        }
                        ...
                        $sale = Sale::where('ctr_id', '=', Input::get('country'))->where('csn_id', Input::get('sale_season'))->where('sltyp_id', '1')->where('sl_catalogue_confirmedby', '!=', null)->where('sl_quality_confirmedby', '!=', null)->where('sl_auction_confirmedby', '!=', null)->get();
                        // if($request->has('seller') && Input::get('seller') != NULL){
                        //     $sale_lots = sale_lots::where('slrid',Input::get('seller'))->where('cbid',Input::get('coffee_buyer'))->whereNotNull('warrant_no')->get();
                        //     $lot_in_sale = sale_lots::where('slrid',Input::get('seller'))->where('cbid',Input::get('coffee_buyer'))->where('lot', $lot)->first();
                        //     $cdetails = coffee_details::where('id', $lot_in_sale->id)->first();
                        ...
                        $csn_season = Input::get('sale_season');
                        return View::make('releaseinstructions', compact('id',
                            'Season', 'country', 'cid', 'csn_season', 'sale', 'CoffeeGrade', 'Warehouse', 'Mill', 'Certification', 'seller', 'sale_lots', 'saleid', 'greensize', 'greencolor', 'greendefects', 'processing', 'screens', 'cupscore', 'rawscore', 'buyer', 'sale_status', 'basket', 'slr', 'sale_cb_id', 'sale_lots', 'transporters', 'trp', 'release_no', 'ref_no', 'rl_date'));
                    }
                    ...
                    $cid        = Input::get('country');
                    $csn_season = Input::get('sale_season');
                    return View::make('releaseinstructions', compact('id', 'Season', 'country', 'cid', 'csn_season', 'sale', 'CoffeeGrade', 'Warehouse', 'Mill', 'Certification', 'seller', 'greensize', 'greencolor', 'greendefects', 'processing', 'screens', 'cupscore', 'rawscore', 'buyer', 'sale_status', 'basket', 'slr', 'sale_cb_id', 'ref_no', 'rl_date'));
                }
            } else {
            ...
            return View::make('releaseinstructions', compact('id', 'Season', 'country', 'Warehouse', 'Mill', 'Certification', 'seller', 'greensize', 'greencolor', 'greendefects', 'processing', 'screens', 'cupscore', 'rawscore', 'buyer', 'sale_status', 'basket', 'slr', 'sale_cb_id', 'ref_no', 'rl_date'));
        }
        ...
        if (null !== Input::get('submitlot')) {
            ...
            for ($i = 0; $i < count($lotid); $i++) {
                ...
                if ($pdetails != null) {
                    $pid = $pdetails->id;
                    purchase::where('id', '=', $pid)
                        ->update(['prc_confirmed_price' => $cprice[$i], 'sst_id' => 1]);
                } else {
                    purchase::insert(['cfd_id' => $lotid[$i], 'prc_confirmed_price' => $cprice[$i], 'sst_id' => 1]);
                }
