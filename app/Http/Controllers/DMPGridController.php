...
class DMPGridController  extends Controller {

    public function stocksDMP (Request $request){
        $query = DMPReport::query();
        $grid = new Grid(
            (new GridConfig)
                ->setDataProvider(
                    new EloquentDataProvider($query)
                )
                ->setName('Stocks_DMP_' . date('d-m-Y'))
                ->setPageSize(50)                
                ->setColumns([ 
                    (new FieldConfig)
                        ->setName('gr_number')
        ...
        return View::make('stocksdmp', compact('id','grid'));
