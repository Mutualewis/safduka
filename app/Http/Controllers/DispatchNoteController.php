...
class DispatchNoteController extends Controller {
    ...
    public function salesDispatchNoteForm (Request $request){
    	$id = null;
    	...
    	return View::make('salesdispatchtnote', compact('id', 'Season', 'country', 'shipmentmonth','client'));

    }


    public function salesDispatchNote (Request $request){
        $id = null;
        $Season = Season::all(['id', 'csn_season']);
        $country = country::all(['id', 'ctr_name', 'ctr_initial']);
        $client = Client::all(['id', 'cl_name']);
        ...
        if (NULL !== Input::get('createsalescontract')){
            ...
            if ($SalesContract !== NULL){
                ...
                Activity::log('Updated salesdispatchtnote ctr_id '. $cid. ' cl_id '. $clid. ' sct_number '. $contract. ' sct_description '. $description.' sct_packages '.$packages.' sct_month '.$spid.' sct_disposal_date '.$disposaldate );
                ...
                Activity::log('Added salesdispatchtnote ctr_id '. $cid. ' cl_id '. $clid. ' sct_number '. $contract. ' sct_description '. $description.' sct_packages '.$packages.' sct_month '.$spid.' sct_disposal_date '.$disposaldate );
            }
            $disposaldate = Input::get('disposaldate');

            return View::make('salesdispatchtnote', compact('id', 'Season', 'country', 'cid', 'contract', 'shipmentmonth','client', 'clid', 'spid', 'disposaldate', 'description', 'packages'));
            ...
            return View::make('salesdispatchtnote', compact('id', 'Season', 'country', 'cid', 'contract', 'shipmentmonth','client', 'clid', 'spid', 'date', 'disposaldate', 'description', 'packages', 'SalesContract'));
