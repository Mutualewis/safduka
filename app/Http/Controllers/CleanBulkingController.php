...
class CleanBulkingController extends Controller {
    ...
    public function bulkingApi(Request $request){
		...
		try{
            ...
            if($st_bulk_details != null){
                ...
                $st_original_bulk_id = $st_bulk_id;
                ...
                StockWarehouse::where('st_bulk_id', '=', $st_bulk_id)
                    ...
                    $prdetails = ProvisionalBulk::where('st_wr_id', $st_bulk_id)->first();
                if ($prdetails != null) {
            ...
            if ($tobeprocessed != null) {
                ...
                // exit();
                if ($stock_details == null) {
                    $st_bulk_id = StockWarehouse::insertGetId([ 'cgr_id' => $grower, 'csn_id' => $stockitemdetails->csn_id, 'st_outturn' => $ref_no, 'st_mark' => $mark, 'st_outturn' => $ref_no, 'usr_id' => $user, 'mt_id' => $material_id, 'st_is_bulk' => 1, 'st_owner_id' => 5, 'warehouse_id' => $warehouse_id]);

                } else {
                    $st_bulk_id = $stock_details->id;
                    StockWarehouse::where('id', '=', $st_bulk_id)
                         ->update([ 'cgr_id' => $grower, 'csn_id' => $stockitemdetails->csn_id, 'st_outturn' => $ref_no, 'st_mark' => $mark, 'st_outturn' => $ref_no, 'usr_id' => $user, 'mt_id' => $material_id, 'st_is_bulk' => 1, 'st_owner_id' => 5, 'warehouse_id' => $warehouse_id]);                 
                }
            ...
            $prdetails = ProvisionalBulk::where('st_wr_id', $st_original_bulk_id)->first();

            if ($prdetails != null) {
                $prid = $prdetails->id;
                ProvisionalBulk::where('id', '=', $prid)
                    ->update(['prcss_id' => $prc, 'ctr_id' => $cid, 'pbk_instruction_number' => $ref_no, 'pbk_weight_in' => $weight_in, 'pbk_reference_name' => $instruction, 'csn_id' => $season , 'st_wr_id' => $st_original_bulk_id]);

                Activity::log('Updated Provisional Bulk information with prid ' . $prid . ' prc ' . $prc . ' ref_no ' . $ref_no . ' weight_in ' . $weight_in );
            } else {
                $prid = ProvisionalBulk::insertGetId(['prcss_id' => $prc, 'pbk_instruction_number' => $ref_no, 'pbk_weight_in' => $weight_in,  'ctr_id' => 1, 'pbk_reference_name' => $instruction, 'pbk_date' => $date, 'prp_id' => 1, 'csn_id' => $season, 'st_wr_id' => $st_original_bulk_id]);
    ...
    public function cleanResultsForm(Request $request)
    {
        ...
        return View::make('cleanresults', compact('id',
            'Season', 'country', 'cid', 'csn_season', 'sale', 'CoffeeGrade', 'warehouse', 'Mill', 'Certification', 'seller', 'sale_lots', 'saleid', 'greensize', 'greencolor', 'greendefects', 'processing', 'screens', 'cupscore', 'rawscore', 'buyer', 'sale_status', 'basket', 'slr', 'sale_cb_id', 'transporters', 'trp', 'release_no', 'date', 'sale_lots_released', 'ref_no', 'rates', 'teams', 'st_id_selected', 'stock_details', 'provisionalbulk', 'material', 'growers', 'packaging', 'bulkinstructions'));
    ...
    public function cleanResults(Request $request)
    {
        ...
        $ref_no = Input::get('ref_no');
            if ($ref_no !== NULL) {
                ...
                $stockview = DB::table('stock_warehouse_st as st')
                ->select('*', 'st.id as stid', 'st.updated_at as date')
                ->leftJoin('provisional_bulk_pbk as pbk', 'st.id', '=', 'pbk.st_wr_id' )
                ->leftJoin('material_mt as mt', 'st.mt_id', '=', 'mt.id')
                ->leftJoin('packaging_pkg as pkg', 'st.pkg_id', '=', 'pkg.id')
                ->leftJoin('stock_locations as sl', 'sl.stid', '=', 'st.id')
                ->where('st.id', '=', $ref_no)
                ->first();

                $stock_id = $stockview->stid;

            }

            $st_bags_instructed = 0;
            $st_pockets_instructed = 0;
            $st_net_weight_instructed = 0;
            ...
            $st_outturn = $stockview->st_outturn;
        ...
        return View::make('cleanresults', compact('id',
            'Season', 'country', 'cid', 'csn_season', 'sale', 'CoffeeGrade', 'Warehouse', 'Mill', 'Certification', 'seller', 'sale_lots', 'saleid', 'greensize', 'greencolor', 'greendefects', 'processing', 'screens', 'cupscore', 'rawscore', 'buyer', 'sale_status', 'basket', 'slr', 'sale_cb_id', 'transporters', 'trp', 'release_no', 'date', 'sale_lots_released', 'ref_no', 'rates', 'teams', 'st_id_selected', 'stock_details', 'packaging'));
    ...
    public function getBulkDetails($process, $bulking_season){
        try {
            ...
            if ($process !== NULL) {
                ...
                $stockview = DB::table('stock_warehouse_st as st')
                ->leftJoin('provisional_bulk_pbk as pbk', 'st.id', '=', "pbk.st_wr_id" )
                ->where('st_outturn', '=', $process)
                ->where('st.csn_id', '=', $bulking_season)
                ->first();
    ...
    public function printResults($ref_no){
        try {
    ...
    public function getResults($process){
        try {
            ...
            if ($process !== NULL) {
                ...
                ->leftJoin('provisional_bulk_pbk as pbrts', 'pbrts.pbk_instruction_number', '=', 'st.st_outturn')
                ->leftJoin('material_mt as mt', 'st.mt_id', '=', 'mt.id')
    ...
    public function addBatch($ref_no, $warehouse, $weigh_scales, $zone, $packaging, $packages_batch, $batch_kilograms, $pallet_kgs, $selectedRow, $selectedColumn, $material, $grower, $sample)
    {   
        ...
        try{
            ...
            $mark = $grower_details->cgr_mark;
            $tare_batch = 0;
            $batch_kilograms_update = 0;
            $packages_batch_update = 0;
            ...
            $batch_net_kilograms = $batch_kilograms - $tare_batch;
            ...
            if ($prdetails != null) {
                ...
                // exit();
                if ($stock_details == null) {
                    $st_bulk_id = StockWarehouse::insertGetId(['cgr_id' => $grower, 'st_outturn' => $outturn, 'st_mark' => $mark, 'st_packages'=>$packages_batch, 'st_net_weight' => $batch_net_kilograms, 'st_bags' => $stock_bags, 'st_pockets' => $stock_pockets, 'usr_id' => $user, 'mt_id' => $material_id, 'st_is_bulk' => 1, 'st_owner_id' => 5, 'warehouse_id' => $warehouse_id, 'st_gross' => $batch_kilograms, 'st_tare' => $tare_batch, 'pkg_id' => $packaging]);

                } else {

                    $packages_batch_update = $stock_details->st_packages + $packages_batch;
                    $batch_kilograms_update = $stock_details->st_net_weight + $batch_net_kilograms;
                    $batch_gross_kilograms_update = $stock_details->st_net_weight + $batch_kilograms;
                    $stock_bags_update = $stock_details->st_bags + $stock_bags;
                    $stock_pockets_update = $stock_details->st_pockets + $stock_pockets;
                    $tare_batch_update = $stock_details->st_tare + $tare_batch;

                    $st_bulk_id = $stock_details->id;
                    StockWarehouse::where('id', '=', $st_bulk_id)
                         ->update(['cgr_id' => $grower, 'st_outturn' => $outturn, 'st_mark' => $mark, 'st_packages'=>$packages_batch_update, 'st_net_weight' => $batch_kilograms_update, 'st_bags' => $stock_bags_update, 'st_pockets' => $stock_pockets_update, 'usr_id' => $user, 'mt_id' => $material_id, 'st_is_bulk' => 1, 'st_owner_id' => 5, 'warehouse_id' => $warehouse_id, 'st_gross' => $batch_gross_kilograms_update, 'st_tare' => $tare_batch_update, 'st_sample' => $sample, 'pkg_id' => $packaging]);                 
                }
                ...
                // exit();
                if ($results_details == null) {
                    $result_id = ProvisionalResults::insertGetId([ 'pr_id' => $ref_no, 'st_wr_id' => $st_bulk_id, 'prts_weight' => $batch_kilograms, 'prts_packages' => $batch_packages]);

                } else {
                    $result_id = $results_details->id;
                    ProvisionalResults::where('id', '=', $st_bulk_id)
                         ->update([ 'pr_id' => $ref_no, 'st_wr_id' => $st_bulk_id, 'prts_weight' => $batch_kilograms_update, 'prts_packages' => $packages_batch_update]);                 
                }
               
                $btid = Batch::insertGetId (
                    ['st_id' => $st_bulk_id, 'btc_weight' => $batch_kilograms, 'btc_tare' => $tare_batch, 'btc_net_weight' => $batch_net_kilograms, 'btc_packages' => $packages_batch, 'btc_bags' => $stock_bags, 'btc_pockets' => $stock_pockets, 'ws_id' => null]);
                ...
                ProvisionalBulk::where('id', '=', $prid)
                    ->update(['pbk_confirmed_by' => $user ]);
                Activity::log('Updated Provisional Bulk information with prid ' . $prid .' ref_no ' . $ref_no . ' weight_in ' . $batch_kilograms );
