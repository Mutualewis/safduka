    ...
    public function processingInstructions(Request $request)
    {
        ...
        if (null !== Input::get('confirminstruction')) {
            ...
            if ($prdetails != null) {
                ...
                $prid = Process::insertGetId(['prcss_id' => $prc, 'pr_instruction_number' => $ref_no, 'pr_weight_in' => $weight_in, 'pr_other_instructions' => $other_instructions, 'ctr_id' => $cid, 'csn_id' => $prc_season , 'pr_reference_name' => $reference, 'pr_date' => $date]);
                $request->session()->flash('alert-success', 'Process Information Updated!!');
            ...
            $processing_type = processing::where('id', $prc)->first();
             ...
             if ($prc == $BULKING_PROCESS) {
                if ($contractID == $INTERNAL_BULK) {
                    $reference_no = Process::where('prcss_id', $prc)->where('pr_instruction_number', 'like', '%' .$COUNTRY_INITIAL. "IB" . '%')->orderBy('pr_instruction_number', 'asc')->pluck('pr_instruction_number');
                    
                } else {
                    $reference_no = Process::where('prcss_id', $prc)->where('pr_instruction_number', 'like', '%' .$COUNTRY_INITIAL. "DI" . '%')->orderBy('pr_instruction_number', 'asc')->pluck('pr_instruction_number');
    ...
    public function processingResults(Request $request)
    {
        ...
        if ($StockView != null) {
            ...
            foreach ($StockView as $key => $value) {
                ...
                $warehouse_ids [] = $value->wr_id;
            }

            if (count(array_unique($warehouse_ids)) === 1 ) {
                $Warehouse = warehouses_region::where('wrid', end($warehouse_ids))->where('ctr_id', Input::get('country'))->where('wrt_id', '1')->get();
            }
