...
class WarrantedVsGRNGridController extends Controller {

  public function stocksWarrantedVsGRNGrid (Request $request){
        $query = StockViewAll::select('*')->whereNotNull('prcid')->whereNull('prcssid');
        ...
        $grid = new Grid(
            (new GridConfig)
                ...
                ->setName('WarrantedGRN_' . date('d-m-Y'))
                ->setPageSize(50)                
                ->setColumns([
                    (new FieldConfig)
                        ->setName('csn_season')
                    ...
                    (new FieldConfig)
                        ->setName('warrant_weight')
                        ->setLabel('Warrant Weight')
                        ->addFilter(
                            (new FilterConfig)
                                ->setName('warrant_weight')
                                ->setOperator(FilterConfig::OPERATOR_LIKE)
                    ...
                    (new FieldConfig)
                        ->setName('st_dispatch_net')
                        ->setLabel('Dispatch Weight')
                        ->addFilter(
                            (new FilterConfig)
                                ->setName('st_dispatch_net')
                                ->setOperator(FilterConfig::OPERATOR_LIKE)
        ...
        return View::make('stockswarrantedvsgrn', compact('id','grid'));
