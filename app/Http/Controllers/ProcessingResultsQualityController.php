    ...
    public function processingResultsQuality(Request $request)
    {
        ...
        if (null !== Input::get('confirmresults')) {
            ...
            try {
                ...
                $process_number = $prdetails->pr_instruction_number;
                ...
                foreach ($process_results as $key => $value) {
                        ...
                        foreach ($stocks_in_bulk as $keyst => $valuest) {
                            ...
                            if ($stock_single_lots->st_net_weight != $valuest->weight && !in_array($valuest->id, $previousStid)) { 
                                ...
                                if ($batch_single_lots != null) {
                                    ...
                                    if ($batch_location_single_lots != null) {
                                        $warehouse_id = $batch_location_single_lots->loc_wr_id;
                                    }
                                ...
                                $previous_stock_breakdown = StockBreakdown::where('st_id', $valuest->id)->whereNull('cn_id')->get();
                                $this->updateOutturnBreakdownBalance($valuest->id, $new_stock_value, $stb_weight, $stid);
                                ...
                                if ($batch_single_lots != null) {
                                    ...
                                    ['bt_id' => $btid, 'loc_wr_id' => $batch_location_single_lots->loc_wr_id, 'loc_row_id' => $batch_location_single_lots->loc_row_id, 'loc_column_id' => $batch_location_single_lots->loc_column_id, 'btc_zone' => $batch_location_single_lots->btc_zone]);
                        ...
                        $this->updateOutturnBreakdown($stid, $StockView, $total_kilos, $batch_kilograms, $total_bric_initial_value, $total_initial_value, $total_kilos_processed, $count_stocks_in_bulk );
                        foreach ($StockView as $keysv => $valuesv) {
                            ...
                            $stock_previous_id = $valuesv->id;


                            $batch_single_lots = Batch::where('st_id', $stock_previous_id)->orderBy('id', 'desc')->first();
                            ...
                            foreach ($stock_item_breakdown as $sibkey => $sibvalue) {
                                ...
                                $fbr_id = $sibvalue->fbr_id;
                                $coffee_details_id = $sibvalue->cfd_id;
                               ...
                               #$stb_value_ratio = ($stock_net/$stock_single_lots->st_net_weight) * ($psbvalue->stb_value)/$stock_bric_value;
                                ...
                                StockBreakdown::insertGetId (
                                     ['st_id' => $stid, 'cfd_id' => $coffee_details_id, 'fbr_id' => $fbr_id, 'stb_weight' => $stb_weight, 'stb_bulk_ratio' => $stb_ratios, 'stb_value_ratio' => $stb_store_ratio, 'stb_value' => $stb_value, 'br_id' => $bric_id, 'cgr_id' => $stb_cgr_id, 'cb_id' => $stb_cb_id]);
                        ...
                        $stlocid = StockLocation::insertGetId (
                        ['bt_id' => $btid, 'loc_row_id' => $value->loc_row, 'loc_wr_id' => $warehouse_id, 'loc_column_id' => $value->loc_column, 'btc_zone' => $value->btc_zone]);
                ...
                $request->session()->flash('alert-warning', 'Something went wrong during the update!!');
            }
            ...
            $header = 'Ibero Coffee Trading Company';
            $ref_no    = Input::get('ref_no');
            $TO = "Ibero Export - Warehouse Department";
            $ATTENTION = "Octavinna/Emmanuel";
            $FROM = "Quality Department";
            $reference = Input::get('reference');
            $contractID = Input::get('contract');
            $contractNumber = SalesContract::where('id', '=', $contractID)->first();
